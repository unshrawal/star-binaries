...
void modeBase::emitDecodedLine(int lineNum, QRgb *data, int width){
  size_t tot = sizeof(int32_t) * 2 + 3 * width;
  zmq::message_t msg(tot); // create a message w/ metadata + 3 channels

  uint8_t *ptr = static_cast<uint8_t *>(msg.data());
  
  std::memcpy(ptr, &lineNum, sizeof(int32_t));
  std::memcpy(ptr+4, &width, sizeof(int32_t));

  // we only need 1 byte for each channel in rgb array
  uint8_t *red = ptr+8;
  uint8_t *green = red+width;
  uint8_t *blue = green+width;
...
void modeBase::combineColors()
{
  ...
  emitDecodedLine(displayLineCounter, pixelArray, activeSSTVParam->numberOfPixels);
...
void modeBase::getLineY(bool evenodd)
{
  ...
  if (evenodd)
  {
    ...
    for (unsigned int i = 0; i < activeSSTVParam->numberOfPixels; i++)
    {
      ...
      blueArrayPtr[i] = (b < 0 ? 0 : b);
      yArrayPtr[i] = (ye > 255 ? 255 : ye);
      yArrayPtr[i] = (ye < 0 ? 0 : ye);
      greenArrayPtr[i] = (yo > 255 ? 255 : yo);
      greenArrayPtr[i] = (yo < 0 ? 0 : yo);
    }
    displayLineCounter++;
  }
  else
  {
    ...
    for (unsigned int i = 0; i < activeSSTVParam->numberOfPixels; i++)
    {
      ...
      redArrayPtr[i] = (r > 255 ? 255 : r < 0 ? 0
                                              : r);
      blueArrayPtr[i] = (b > 255 ? 255 : b < 0 ? 0
                                               : b);
      yArrayPtr[i] = (ye > 255 ? 255 : ye < 0 ? 0
                                              : ye);
    }
  }
  displayLineCounter++;
